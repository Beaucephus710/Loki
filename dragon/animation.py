...
class DragonAnimator:
    ...
    def _draw_background(self, draw: ImageDraw.ImageDraw, frame: int) -> None:
        draw.rectangle((0, self.height - 55, self.width, self.height), fill=(30, 24, 40))
        draw.line((0, self.height - 55, self.width, self.height - 55), fill=(97, 63, 54), width=2)
        for i in range(16):
    ...
    def _draw_status(self, draw: ImageDraw.ImageDraw, state: DragonState) -> None:
        draw.text((12, 10), _ascii_safe(f"LOKI - {state.title}"), font=self._font, fill="white")
        draw.text(
            (12, 28),
            _ascii_safe(f"Level {state.level}  XP {state.xp}  Mood: {state.mood_name}"),
            font=self._font,
            fill=(180, 210, 255),
        )
    ...
    def _draw_egg(self, draw: ImageDraw.ImageDraw, x: int, y: int, state: DragonState, frame: int) -> None:
        draw.ellipse((x - 55, y - 75, x + 55, y + 75), fill=(75, 190, 165), outline=(225, 255, 235), width=3)
        ...
        for n in range(crack_count):
            off = n * 11 - 17
            draw.line([(x + off, y - 22), (x + off + 8, y - 3), (x + off - 3, y + 17)], fill=(32, 70, 68), width=2)

        pulse = int((math.sin(frame / 3.0) + 1) * 3)
        draw.arc((x - 65 - pulse, y - 85 - pulse, x + 65 + pulse, y + 85 + pulse), 190, 350, fill=(255, 210, 80), width=2)
        draw.text((x - 64, y + 95), _ascii_safe("Keep Loki company to hatch!"), font=self._font, fill=(255, 220, 130))

    def _draw_dragon(self, draw: ImageDraw.ImageDraw, x: int, y: int, state: DragonState, frame: int) -> None:
        ...
        draw.polygon([(x - s(28), y - s(25)), (x - s(85), y - s(65) - wing_flap), (x - s(60), y + s(25))], fill=(42, 105, 130), outline=(205, 245, 255))
        draw.polygon([(x + s(28), y - s(25)), (x + s(85), y - s(65) - wing_flap), (x + s(60), y + s(25))], fill=(42, 105, 130), outline=(205, 245, 255))

        draw.line((x - s(25), y + s(35), x - s(90), y + s(55)), fill=body_color, width=max(2, s(14)))
        draw.ellipse((x - s(42), y - s(18), x + s(42), y + s(65)), fill=body_color, outline=(225, 255, 240), width=2)
        draw.ellipse((x - s(37), y - s(70), x + s(37), y), fill=body_color, outline=(225, 255, 240), width=2)

        draw.polygon([(x - s(24), y - s(61)), (x - s(15), y - s(87)), (x - s(7), y - s(58))], fill=(255, 230, 155))
        draw.polygon([(x + s(24), y - s(61)), (x + s(15), y - s(87)), (x + s(7), y - s(58))], fill=(255, 230, 155))
        ...
        if state.mood_name == "grumpy":
            draw.line((x - s(15), mouth_y, x + s(15), mouth_y - s(4)), fill=(30, 30, 45), width=2)
        else:
            draw.arc((x - s(15), mouth_y - s(5), x + s(15), mouth_y + s(12)), 0, 180, fill=(30, 30, 45), width=2)

        if state.stage == "adult":
            draw.text((x - s(45), y + s(82)), _ascii_safe("Ancient flame awakens"), font=self._font, fill=(255, 170, 70))
